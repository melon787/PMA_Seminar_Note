...
\end{definition}
\begin{theorem} \label{thm:property_closed}
...
\begin{proof}\,
\begin{enumerate} 
\item[(a)] Suppose $A$ and $B$ are disjoint closed subsets of metric space $X$. 
Since $A = \overline{A}$ and $B= \overline{B}$ by the Theorem \ref{thm:property_closed}, we have $\overline{A} \cap B=A \cap \overline{B}=A \cap B = \varnothing$.
This proves (a).
\item[(b)] Let $A$ and $B$ be disjoint open subsets of $X$.
Since $A \cap B = \varnothing$, we have $A \subset B^c$. 
Since $B^c$ is closed (notice $B$ is open), the Theorem \ref{thm:property_closed} implies that $\overline{A} \subset B^c$, hence $\overline{A} \cap B = \varnothing$.
By symmetry, a same argument proves that $A \cap \overline{B} = \varnothing$.
...
\item[(d)] Let $X$ be a connected a connected metric space.
Suppose $p$ and $q$ are two distinct points in $X$.
...
\end{align*}
It follows from (c) that $A_{\delta}$ and $B_{\delta}$ are separated if $0<\delta<r$.
...
If $\delta_1 \neq \delta_2$ then $f(\delta_1) \neq f(\delta_2)$, hence $f:(0,r) \longrightarrow X$ 1-1 function from $(0,r)$ to $X$.
Since $(0,r)$ is uncountable subset of $\R$, we conclude that $X$ is uncountable.
\end{enumerate}
\end{proof}
