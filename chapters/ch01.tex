...
\end{definition}
\begin{definition} \label{def:sup_inf}
...
\begin{lemma}\label{greatest integer} \footnote{This lemma has been added for convenience and does not appear in the original text.}
\begin{enumerate}
...
We presuerve the original order in $\R$, and define
\[
-\infty<x<+\infty
\]
for every $x \in \R$.
...
Then by the Corollary to Theorem \ref{thm:nth_square_root}, we obtain
\begin{align*}
b^{r+s}
&=b^{\frac{m}{n}+\frac{p}{q}}
=b^{\frac{mq+np}{nq}}\\
